...
function setupDatabaseIPC() {
  ...
  // Verse operations
  ipcMain.handle('db:loadVerses', async (event, { versionId, bookId, chapter }: { versionId: string; bookId: number; chapter: number }) => {
    try {
  ...
  // Search operations
  ipcMain.handle('db:searchVerses', async (event, { query, versionId }: { query: string; versionId?: string }) => {
    try {
  ...
  ipcMain.handle('db:loadSongs', async (event, { search, limit = 50 }: { search?: string; limit?: number }) => {
    try {
      const where = search ? {
        OR: [
  ...
  ipcMain.handle('db:setSetting', async (event, { key, value, type = 'string', category }: { key: string; value: string; type?: string; category?: string }) => {
    try {
