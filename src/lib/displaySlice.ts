...
export const refreshDisplays = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error instanceof Error ? error.message : 'Failed to get displays');
    }
  }
);
...
export const captureDisplay = createAsyncThunk(
  ...
  async (displayId: number, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error instanceof Error ? error.message : 'Failed to capture display');
    }
  }
);
...
export const testDisplay = createAsyncThunk(
  ...
  async (displayId: number, { rejectWithValue }) => {
    try {
      const result = await window.electronAPI?.invoke('display:testDisplay', displayId);
      return result;
    } catch (error) {
      return rejectWithValue(error instanceof Error ? error.message : 'Failed to test display');
    }
  }
);
...
export const saveDisplaySettings = createAsyncThunk(
  ...
  async (settings: Partial<DisplaySettings>, { rejectWithValue }) => {
    try {
      const result = await window.electronAPI?.invoke('display:saveSettings', settings);
      return result;
    } catch (error) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(refreshDisplays.fulfilled, (state, action) => {
        ...
        if (action.payload) {
          ...
          if (state.settings.selectedLiveDisplayId) {
            const stillExists = displays.find((d: DisplayInfo) => d.id === state.settings.selectedLiveDisplayId);
            if (!stillExists) {
      ...
      .addCase(captureDisplay.fulfilled, (state, action) => {
        ...
        state.capturingDisplays = state.capturingDisplays.filter(id => id !== displayId);
        if (screenshot) {
...
// Selectors
export const selectDisplays = (state: { display: DisplayState }) => state.display.displays;
export const selectPrimaryDisplay = (state: { display: DisplayState }) => state.display.primaryDisplay;
export const selectSecondaryDisplay = (state: { display: DisplayState }) => state.display.secondaryDisplay;
export const selectSelectedLiveDisplay = (state: { display: DisplayState }) => state.display.selectedLiveDisplay;
export const selectDisplaySettings = (state: { display: DisplayState }) => state.display.settings;
export const selectHasMultipleDisplays = (state: { display: DisplayState }) => state.display.displays.length > 1;
export const selectDisplayCount = (state: { display: DisplayState }) => state.display.displays.length;
export const selectDisplayLoading = (state: { display: DisplayState }) => state.display.isLoading;
export const selectDisplayError = (state: { display: DisplayState }) => state.display.error;
export const selectDisplayCaptures = (state: { display: DisplayState }) => state.display.displayCaptures;
export const selectCapturingDisplays = (state: { display: DisplayState }) => state.display.capturingDisplays;
export const selectDisplayCapture = (displayId: number) => (state: { display: DisplayState }) => 
  state.display.displayCaptures[displayId];
export const selectIsCapturing = (displayId: number) => (state: { display: DisplayState }) => 
