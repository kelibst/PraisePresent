...
export const refreshDisplays = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      return await window.electron.displayManager.getDisplays();
    } catch (error) {
...
export const captureDisplay = createAsyncThunk(
  ...
  async (displayId: number, { rejectWithValue }) => {
    try {
      const screenshot = await window.electron.displayManager.captureDisplay(displayId);
      return { displayId, screenshot };
    } catch (error) {
      return rejectWithValue(
...
export const testDisplay = createAsyncThunk(
  ...
  async (displayId: number, { rejectWithValue }) => {
    try {
      return await window.electron.displayManager.testDisplay(displayId);
    } catch (error) {
      return rejectWithValue(
...
export const saveDisplaySettings = createAsyncThunk(
  ...
  async (settings: Partial<DisplaySettings>, { rejectWithValue }) => {
    try {
      return await window.electron.displayManager.saveSettings(settings);
    } catch (error) {
      return rejectWithValue(
...
// Async thunk to create live display
export const createLiveDisplay = createAsyncThunk(
  "display/createLiveDisplay",
  async (displayId: number, { rejectWithValue, getState }) => {
    try {
      ...
      return rejectWithValue(
        error instanceof Error ? error.message : "Failed to create live display"
      );
    }
  }
);
...
const displaySlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Create live display
      .addCase(createLiveDisplay.fulfilled, (state, action) => {
        const displayId = action.payload;
        state.settings.selectedLiveDisplayId = displayId;
        state.settings.isLiveDisplayActive = true;
        state.selectedLiveDisplay = state.displays.find(d => d.id === displayId) || null;
      })
      .addCase(createLiveDisplay.rejected, (state, action) => {
        state.error = action.payload as string;
        state.settings.isLiveDisplayActive = false;
      });
  },
});
