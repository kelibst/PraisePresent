...
const SimpleSongEditor: React.FC<SimpleSongEditorProps> = ({ isOpen, onClose, song, mode }) => {
	...
	useEffect(() => {
		if (song && mode === 'edit') {
			...
			if (song.structure?.slides) {
				setSlides(song.structure.slides);
			} else if (song.lyrics) {
				// Auto-parse lyrics into slides
	...
	// Parse lyrics into slides (simplified version)
	...
	const handleSaveSong = async () => {
		...
		try {
			...
			// Prepare song data (only include fields that exist in the schema)
			const songData = {
				title: songInfo.title.trim(),
				artist: songInfo.artist.trim() || undefined,
				author: songInfo.author.trim() || undefined,
				lyrics: reconstructedLyrics,
				key: songInfo.key || undefined,
				tempo: songInfo.tempo,
				category: songInfo.category,
				ccliNumber: songInfo.ccliNumber.trim() || undefined,
				copyright: songInfo.copyright.trim() || undefined,
				notes: songInfo.notes.trim() || undefined,
				// Store structure in chords field as JSON (temporary solution)
				chords: JSON.stringify(songStructure),
			};
	...
	const saveSlide = (slideData: SongSlide) => {
		if (editingSlide) {
...
export default SimpleSongEditor; 